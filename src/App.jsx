...
function makeSB(url, key) {
  ...
  return {
    ...
    async post(table, data) {
      const r = await fetch(`${base}/rest/v1/${table}`, { method:"POST", headers:h, body:JSON.stringify(data) });
      if (!r.ok) throw new Error(await r.text());
      return r.json();
    },
    async patch(table, id, data) {
      const r = await fetch(`${base}/rest/v1/${table}?id=eq.${id}`, { method:"PATCH", headers:h, body:JSON.stringify(data) });
      if (!r.ok) throw new Error(await r.text());
      return r.json();
    },
    async del(table, id) {
      const r = await fetch(`${base}/rest/v1/${table}?id=eq.${id}`, { method:"DELETE", headers:h });
      if (!r.ok) throw new Error(await r.text());
    },
    async upsertTasks(rows) {
      const r = await fetch(`${base}/rest/v1/tasks`, {
        method:"POST",
...
function Badge({label,bg,color}){
  ...
  return <span style={{background:bg||(s?.bg)||C.goldBg,color:color||(s?.color)||C.gold,borderRadius:20,padding:"3px 10px",fontSize:11,fontWeight:700,display:"inline-flex",alignItems:"center",gap:5,whiteSpace:"nowrap"}}>{s&&<span style={{width:6,height:6,borderRadius:"50%",background:s.dot}}/>}{label}</span>;
}
...
-- Allow public access (private app)
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Nunito',sans-serif",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:32,maxWidth:560,width:"100%",boxShadow:"0 8px 40px rgba(0,0,0,0.08)"}}>
        ...
        <div style={{fontSize:13,color:C.textMuted,marginBottom:24,letterSpacing:"0.08em",textTransform:"uppercase",fontSize:10}}>One-time database setup</div>
        ...
        <div style={{background:C.goldBg,border:`1px solid ${C.goldBorder}`,borderRadius:12,padding:16,marginBottom:20,fontSize:13,color:C.textMid,lineHeight:2}}>
          ...
          2. Open <strong>SQL Editor</strong> → paste & run the SQL below<br/>
          3. Go to <strong>Project Settings → API</strong> → copy URL + anon key<br/>
          4. Paste them here → your data saves forever ✓
        </div>

        <div style={{background:"#1a1a2e",borderRadius:10,padding:14,marginBottom:20,fontSize:11,color:"#a8dadc",fontFamily:"monospace",lineHeight:1.8,overflowX:"auto",whiteSpace:"pre",userSelect:"all",cursor:"text"}}>
          {SQL}
        </div>

        <div style={{marginBottom:12}}>
          <label style={LBL}>Supabase Project URL</label>
          <input style={INP} placeholder="https://xxxxxxxxxxxx.supabase.co" value={url} onChange={e=>setUrl(e.target.value)}/>
        </div>
        <div style={{marginBottom:20}}>
          <label style={LBL}>Supabase Anon / Public Key</label>
          <input style={INP} placeholder="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9…" value={key} onChange={e=>setKey(e.target.value)} type="password"/>
        </div>
...
export default function App(){
  ...
  const blankS={customer:"",product:"Business Clarity Session",amount:"",currency:"₦ NGN",date:today(),payStatus:"Paid",delivery:"Delivered"};
  ...
  async function addIntel(){
    ...
    try{const [row]=await sb.post('intel',{data:inf});setIntel([fromRow(row),...intel]);setInf(blankI);setShowIF(false);toast$("Session notes saved!");}
    catch(e){toast$(e.message,"err");}
  }
  ...
  async function del(table,id,setter,list,extra){
  ...
  // ── COMPUTED ────────────────────────────────────────────────────────────────
  const totalNGN=sales.filter(s=>s.payStatus==="Paid").reduce((a,s)=>a+toNGN(s.amount,s.currency),0);
  ...
  const filteredIntel=intelFilter==="All"?intel:intel.filter(i=>i.tags?.includes(intelFilter));
  const q=search.toLowerCase();
  const fLeads=leads.filter(l=>!q||[l.name,l.phone,l.product,l.notes].some(x=>x?.toLowerCase().includes(q)));
  const fSales=sales.filter(s=>!q||s.customer?.toLowerCase().includes(q));
  const fCustomers=customers.filter(c=>!q||[c.name,c.phone,c.city].some(x=>x?.toLowerCase().includes(q)));
  const fProcurement=procurement.filter(p=>!q||[p.name,p.product,p.notes].some(x=>x?.toLowerCase().includes(q)));
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Nunito',sans-serif",color:C.text}}>
      ...
      <div style={{padding:"20px 16px",maxWidth:900,margin:"0 auto"}} className="fade">
        ...
        {tab==="overview"&&(
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10,marginBottom:16}}>
              {[
                {label:"Active Leads",value:hotLeads.length,color:C.orange,bg:C.orangeBg,icon:"🔥"},
                {label:"Paid Clients",value:leads.filter(l=>l.status==="Paid").length,color:C.green,bg:C.greenBg,icon:"💰"},
                {label:"Customers",value:customers.length,color:C.blue,bg:C.blueBg,icon:"👥"},
                {label:"Sessions Today",value:todayBookings.length,color:C.purple,bg:C.purpleBg,icon:"📅"},
                {label:"Overdue Followups",value:overdueLeads.length,color:C.red,bg:C.redBg,icon:"⚠"},
                {label:"Procurement",value:procurement.length,color:C.teal,bg:C.tealBg,icon:"📦"},
              ].map((k,i)=>(
                <div key={i} style={{background:k.bg,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px 14px",textAlign:"center"}} className="fade">
              ...
              <Card>
                ...
                {sales.slice(0,5).map(s=>(
                  <div key={s.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
                    ...
                    <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:700,color:C.green}}>{fmtAmt(s.amount,s.currency)}</div>
                  </div>
                ))}
          ...
          <div>
            ...
            {showLF&&(
              <Card style={{marginBottom:16,border:`1px solid ${C.goldBorder}`,background:C.goldBg}} className="pop">
                ...
                <Row><F label="Full Name *"><Inp placeholder="Aisha Musa" value={lf.name} onChange={e=>setLf({...lf,name:e.target.value})}/></F><F label="Phone *"><Inp placeholder="08012345678" value={lf.phone} onChange={e=>setLf({...lf,phone:e.target.value})}/></F></Row>
                ...
                <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
                  <label style={{display:"flex",alignItems:"center",gap:8,fontSize:13,color:C.textMid,cursor:"pointer"}}>
                    <input type="checkbox" checked={lf.auditSent} onChange={e=>setLf({...lf,auditSent:e.target.checked})} style={{width:16,height:16,accentColor:C.gold}}/>
                    Audit PDF Sent
                  </label>
                </div>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
              {["All",...LEAD_STATUSES].map(s=>{
                ...
                return <button key={s} style={{background:LEAD_STYLE[s]?.bg||C.goldBg,color:LEAD_STYLE[s]?.color||C.gold,border:"none",borderRadius:20,padding:"5px 12px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>{s} ({count})</button>;
              })}
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {fLeads.map(lead=>{
                ...
                return(
                  <div key={lead.id} className="row-card" style={{background:C.card,border:`1px solid ${overdue?C.red:C.border}`,borderRadius:14,overflow:"hidden"}}>
                    <div onClick={()=>setExpandedLead(exp?null:lead.id)} style={{padding:"13px 16px",display:"flex",alignItems:"center",gap:12}}>
                      <div style={{width:9,height:9,borderRadius:"50%",background:LEAD_STYLE[lead.status]?.dot||C.textDim,flexShrink:0}}/>
                      ...
                      <div style={{borderTop:`1px solid ${C.border}`,padding:"14px 16px",background:C.bg}}>
                        ...
                        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                          {LEAD_STATUSES.filter(s=>s!==lead.status).map(s=>(
                            <button key={s} className="qbtn" onClick={()=>quickLeadStatus(lead.id,s)} style={{background:LEAD_STYLE[s]?.bg,color:LEAD_STYLE[s]?.color}}>→ {s}</button>
                          ))}
                          <button className="qbtn" onClick={()=>markAuditSent(lead.id)} style={{background:C.tealBg,color:C.teal}}>✓ Audit Sent</button>
                          <button className="del-btn" onClick={()=>del('leads',lead.id,setLeads,leads,()=>setExpandedLead(null))}>×</button>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              {fLeads.length===0&&<div style={{textAlign:"center",padding:40,color:C.textMuted,fontSize:14}}>No leads found. Add your first lead above.</div>}
            </div>
          </div>
        )}
        ...
        {tab==="sales"&&(
          <div>
            <SectionHead title="Sales & Delivery" sub={`${fmtNGN(totalNGN)} confirmed · ${sales.length} transactions`} action={<GoldBtn onClick={()=>setShowSF(!showSF)}>+ Record Sale</GoldBtn>}/>
            {showSF&&(
              <Card style={{marginBottom:16,border:`1px solid ${C.goldBorder}`,background:C.goldBg}} className="pop">
                <div style={{fontWeight:800,marginBottom:14,color:C.gold}}>New Sale</div>
                <Row><F label="Customer *"><Inp placeholder="Hajiya Binta" value={sf.customer} onChange={e=>setSf({...sf,customer:e.target.value})}/></F><F label="Product"><Sel value={sf.product} onChange={e=>setSf({...sf,product:e.target.value})}>{PRODUCTS.map(p=><option key={p}>{p}</option>)}</Sel></F></Row>
                ...
                <Row><F label="Date"><Inp type="date" value={sf.date} onChange={e=>setSf({...sf,date:e.target.value})}/></F><F label="Payment"><Sel value={sf.payStatus} onChange={e=>setSf({...sf,payStatus:e.target.value})}>{["Paid","Pending","Overdue"].map(s=><option key={s}>{s}</option>)}</Sel></F></Row>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {fSales.map(s=>(
                <div key={s.id} className="row-card" style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"13px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:3}}>
                      <span style={{fontWeight:700,fontSize:14}}>{s.customer}</span>
                      <Badge label={s.payStatus} bg={s.payStatus==="Paid"?C.greenBg:C.orangeBg} color={s.payStatus==="Paid"?C.green:C.orange}/>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:14}}>
                    <div style={{textAlign:"right"}}>
                      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:700,color:C.green}}>{fmtAmt(s.amount,s.currency)}</div>
                      {s.currency!=="₦ NGN"&&<div style={{fontSize:10,color:C.textMuted}}>≈ {fmtNGN(toNGN(s.amount,s.currency))}</div>}
                    </div>
                    <button className="del-btn" onClick={()=>del('sales',s.id,setSales,sales)}>×</button>
                  </div>
                </div>
              ))}
              {fSales.length===0&&<div style={{textAlign:"center",padding:40,color:C.textMuted,fontSize:14}}>No sales yet. Record your first sale above.</div>}
            </div>
          </div>
        )}
          ...
          <div>
            ...
            {showCF&&(
              <Card style={{marginBottom:16,border:`1px solid ${C.goldBorder}`,background:C.goldBg}} className="pop">
                ...
                <Row><F label="Full Name *"><Inp placeholder="Hajiya Binta" value={cf.name} onChange={e=>setCf({...cf,name:e.target.value})}/></F><F label="Phone *"><Inp placeholder="08012345678" value={cf.phone} onChange={e=>setCf({...cf,phone:e.target.value})}/></F></Row>
              ...
              {fCustomers.map(c=>(
                <div key={c.id} className="row-card" style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"13px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <div style={{textAlign:"right"}}>
                    ...
                    <button className="del-btn" onClick={()=>del('customers',c.id,setCustomers,customers)}>×</button>
                  </div>
                </div>
              ))}
          ...
          <div>
            ...
            {showBF&&(
              <Card style={{marginBottom:16,border:`1px solid ${C.goldBorder}`,background:C.goldBg}} className="pop">
                ...
                <Row><F label="Client Name *"><Inp placeholder="Fatima Yusuf" value={bf.client} onChange={e=>setBf({...bf,client:e.target.value})}/></F><F label="Phone"><Inp placeholder="08023456789" value={bf.phone} onChange={e=>setBf({...bf,phone:e.target.value})}/></F></Row>
                ...
                {bf.date&&bf.time&&bookings.find(b=>b.date===bf.date&&b.time===bf.time&&b.status!=="Cancelled")&&(
                  <div style={{background:C.redBg,border:`1px solid ${C.red}`,borderRadius:8,padding:"8px 12px",fontSize:12,color:C.red,marginBottom:12,fontWeight:700}}>
                    ⚠ Time clash with {bookings.find(b=>b.date===bf.date&&b.time===bf.time)?.client} — choose a different time
                  </div>
                )}
                ...
                {bookings.filter(b=>b.date===date).sort((a,b2)=>a.time?.localeCompare(b2.time)).map(b=>(
                  <div key={b.id} className="row-card" style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 16px",marginBottom:6,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    <div style={{display:"flex",gap:8,alignItems:"center"}}>
                      ...
                      <button className="del-btn" onClick={()=>del('bookings',b.id,setBookings,bookings)}>×</button>
                    </div>
                  </div>
                ))}
              </div>
            ))}
            {bookings.length===0&&<div style={{textAlign:"center",padding:40,color:C.textMuted,fontSize:14}}>No bookings yet.</div>}
          </div>
        )}
        ...
        {tab==="cohorts"&&(
          <div>
            <SectionHead title="Cohort Phases" sub="Bootcamp & Life Sessions tracking" action={<GoldBtn onClick={()=>setShowCoF(!showCoF)}>+ New Cohort</GoldBtn>}/>
            {showCoF&&(
              <Card style={{marginBottom:16,border:`1px solid ${C.goldBorder}`,background:C.goldBg}} className="pop">
                ...
                return(
                  <Card key={co.id}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
                      ...
                      <button className="del-btn" onClick={()=>del('cohorts',co.id,setCohorts,cohorts)}>×</button>
                    </div>
        ...
        {tab==="intel"&&(
          <div>
            ...
            <Card style={{marginBottom:16,background:C.goldBg,border:`1px solid ${C.goldBorder}`}}>
              <div style={{fontSize:11,fontWeight:700,color:C.gold,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:10}}>🧠 Top Pain Points Across All Clients</div>
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                {[...new Set(intel.flatMap(i=>i.tags||[]))].map(tag=>{
                  const count=intel.filter(i=>i.tags?.includes(tag)).length;
                ...
                {intelFilter!=="All"&&<span style={{background:C.redBg,color:C.red,borderRadius:20,padding:"4px 12px",fontSize:12,fontWeight:700,cursor:"pointer"}} onClick={()=>setIntelFilter("All")}>× Clear filter</span>}
              </div>
              <div style={{marginTop:10,fontSize:12,color:C.textMuted,fontStyle:"italic"}}>💡 These are your next product ideas. The most repeated problems = highest demand.</div>
            </Card>
            {showIF&&(
              <Card style={{marginBottom:16,border:`1px solid ${C.goldBorder}`,background:C.goldBg}} className="pop">
                <div style={{fontWeight:800,marginBottom:14,color:C.gold}}>New Session Notes</div>
                <Row><F label="Client Name *"><Inp placeholder="Fatima Yusuf" value={inf.client} onChange={e=>setInf({...inf,client:e.target.value})}/></F><F label="Date"><Inp type="date" value={inf.date} onChange={e=>setInf({...inf,date:e.target.value})}/></F></Row>
                ...
                <F label="Pain Point Tags — tap to select">
                  <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:4}}>
                    {PAIN_TAGS.map(tag=>{
                      const sel=inf.tags.includes(tag);
                      return <span key={tag} className="tag-chip" onClick={()=>toggleTag(tag)} style={{background:sel?C.goldBg:"#F5F0EA",color:sel?C.gold:C.textMuted,border:`1px solid ${sel?C.goldBorder:C.border}`}}>{sel?"✓ ":""}{tag}</span>;
                    })}
                  </div>
                </F>
                  ...
                  <div key={i.id} className="row-card" style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,overflow:"hidden"}}>
                    <div onClick={()=>setExpandedIntel(exp?null:i.id)} style={{padding:"13px 16px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      <div style={{flex:1}}>
                        ...
                        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:8}}>
                          {(i.tags||[]).map(tag=><span key={tag} style={{background:C.goldBg,color:C.gold,borderRadius:20,padding:"2px 10px",fontSize:11,fontWeight:700}}>{tag}</span>)}
                        </div>
                      </div>
                    ...
                    {exp&&(
                      <div style={{borderTop:`1px solid ${C.border}`,padding:"14px 16px",background:C.bg}}>
                        ...
                        <button className="del-btn" onClick={()=>del('intel',i.id,setIntel,intel,()=>setExpandedIntel(null))}>× Delete</button>
                      </div>
                    )}
                  </div>
                );
        ...
        {tab==="procurement"&&(
          <div>
            <SectionHead title="Procurement Inquiries" sub={`${procurement.length} inquiries tracked · China sourcing demand`} action={<GoldBtn onClick={()=>setShowPF(!showPF)}>+ New Inquiry</GoldBtn>}/>
            <Card style={{marginBottom:16,background:"linear-gradient(135deg,#EAF4FB,#F0F8FF)",border:`1px solid ${C.blue}33`}}>
              ...
              <div style={{fontSize:13,color:C.textMid,lineHeight:1.7}}>China imports to Nigeria surged <strong>37%</strong> in 2025 as Dubai & Turkey routes become harder. You are positioned perfectly. Track every inquiry here — high demand in any product category = your next procurement service offering.</div>
            </Card>
            {showPF&&(
              <Card style={{marginBottom:16,border:`1px solid ${C.goldBorder}`,background:C.goldBg}} className="pop">
                <div style={{fontWeight:800,marginBottom:14,color:C.gold}}>New Procurement Inquiry</div>
                <Row><F label="Client Name *"><Inp placeholder="Maryam Garba" value={pf.name} onChange={e=>setPf({...pf,name:e.target.value})}/></F><F label="Phone"><Inp placeholder="08045671234" value={pf.phone} onChange={e=>setPf({...pf,phone:e.target.value})}/></F></Row>
                <Row><F label="Product / Category *"><Inp placeholder="Kitchen appliances, fabric, electronics…" value={pf.product} onChange={e=>setPf({...pf,product:e.target.value})}/></F><F label="Quantity"><Inp placeholder="50 units / 200 yards" value={pf.qty} onChange={e=>setPf({...pf,qty:e.target.value})}/></F></Row>
                ...
                <F label="Notes — their concerns, supplier needs, anything important"><Txt placeholder="Are they afraid of scams? Do they need verified supplier?" value={pf.notes} onChange={e=>setPf({...pf,notes:e.target.value})}/></F>
            ...
            <input style={{...INP,marginBottom:12}} placeholder="🔍  Search procurement inquiries…" value={search} onChange={e=>setSearch(e.target.value)}/>
                ...
                <div key={p.id} className="row-card" style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"13px 16px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                    ...
                    <div style={{display:"flex",gap:8,alignItems:"center"}}>
                      ...
                      <button className="del-btn" onClick={()=>del('procurement',p.id,setProcurement,procurement)}>×</button>
                    </div>
                  </div>
